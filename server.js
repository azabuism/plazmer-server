...
class GameRoom {
    ...
    updateInvaders() {
        ...
        gs.invaders = gs.invaders.filter(inv => {
            ...
            else if (inv.phase === 'idle') { if (inv.canShoot && Math.random() < 0.01) { gs.invaderBullets.push({ x: inv.x, y: inv.y, vx: 0, vy: 3 }); } if (inv.kamikaze && gs.fighter.alive) { inv.phase = 'attack'; } if (inv.heals && gs.time % 60 === 0) { gs.invaders.forEach(other => { if (other !== inv && Math.hypot(other.x - inv.x, other.y - inv.y) < 50) { other.hp = Math.min(UNITS[other.type].hp, other.hp + 2); } }); } }
    ...
    hitFighter() { const gs = this.gameState; gs.fighter.lives--; if (gs.fighter.lives > 0) { gs.fighter.invincible = 180; gs.fighter.x = GAME_W / 2; gs.fighter.y = GAME_H - 60; this.broadcast('fighterHit', { lives: gs.fighter.lives }); } else { gs.fighter.alive = false; } }
